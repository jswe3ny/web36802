<script lang="ts">
 ...
 $: totalReturn = data.totalReturn;
  ...
  //   totalReturn += data.displayData[i].totalReturn
  // } 
...
{#if data.displayData.length > 0}
  <div class="mx-auto max-h-[500px] overflow-y-auto max-w-3xl">

   
    <table class="table max-w-3xl mx-auto">
      <!-- head -->
      <thead>
        <tr >
          <th >Ticker</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td></td>
          {#if totalReturn < 0}
                <td class="hidden text-[#dc2626] sm:table-cell">{formatter.format(totalReturn)}</td>  
                {:else}
                <td class="hidden text-[#16A34A] sm:table-cell">+{formatter.format(totalReturn)}</td> 
